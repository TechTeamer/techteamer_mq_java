...
test $build_results -ge 0 && echo "Build failed!" && exit $build_results
...
test $unit_test_results -ge 0 && echo "Unit tests failed!" && exit $unit_test_results
...
test $code_quality_results -ge 0 && echo "Code quality check failed!" && exit $code_quality_results
